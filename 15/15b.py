...
class Battle:
    ...
    hit_points: dict[tuple, int]  # hit_points of units in battle : (x, y) -> health
    ...
    damage_map = {
        Tile.elf.value: None,
        Tile.goblin.value: GOBLIN_ATK,
    }
    # helper template for BFS, no nodes visited
    visited_clean_template: list[list[bool]]

    def __init__(self, battlefield_lines: Iterable[str] = sys.stdin):
        self.battlefield = [list(line.rstrip()) for line in battlefield_lines]
    ...
    def attack(self, x, y) -> bool:
        ...
        self.hit_points[opponent] -= self.damage_map[self.battlefield[x][y]]
    ...
    def victor(self) -> str | None:
        if self.one_sided():
            # get any unit still standing
            (x, y) = next(iter(self.hit_points.keys()))
            return self.battlefield[x][y]

    def unit_count(self) -> Counter:
        return Counter(self.battlefield[x][y] for (x, y) in self.units)
    ...
    def commence(self):
        ...
        while True:
            ...
            if self.one_sided():
                break
        outcome = (round - 1) * sum(self.hit_points.values())
        return outcome
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(L, R)):
        battle = Battle(field)
        battle.damage_map["E"] = i

        before = battle.unit_count()["E"]
        outcome = battle.commence()
        after = battle.unit_count()["E"]
        if after == before:
            print("Elven dmg", i)
